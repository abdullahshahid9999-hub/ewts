...
## IMPORTANT conflict flagged for DASHBOARD-INVENTORY-PAYABLE-PROMPT.md
My earlier session fixed group-ticket seat decrement at BOOKING CREATION time (`app/api/agent/bookings/route.ts`, transaction decrementing `GroupFlight.seats` on POST). This new prompt's Part 2/3 spec says seats + payable should both move at ISSUE time instead (status → 'issued'), not creation. These conflict — if Part 2 is implemented as written without touching my earlier code, seats will decrement TWICE (once at create, once at issue). Whoever picks up DASHBOARD-INVENTORY-PAYABLE-PROMPT.md must first REMOVE the create-time decrement I added before adding the issue-time one, not layer on top of it. Same applies to the cancellation-restores-seat logic I added — that timing assumption changes too if decrement moves to issue-time (a 'pending' or 'issue_requested' booking that gets cancelled never touched seats in the first place under the new model).
Deleted my SALES-DASHBOARD-PROMPT.md — DASHBOARD-INVENTORY-PAYABLE-PROMPT.md (built by a concurrent session) supersedes it with a more precise, correct spec (it correctly identifies the sellPrice-minus-commission-owed-at-issue business rule instead of guessing).

...
export default async function ToursPage({ searchParams }: { searchParams: Promise<{ q?: string; tier?: string; airline?: string; duration?: string; featured?: string; adults?: string; children?: string; infants?: string }> }) {
  const sp = await searchParams;
  const { q, tier, airline, duration, featured } = sp;
  const [packages, facets] = await Promise.all([getPackages(q, tier, airline, duration, featured), getToursFacets()]);
                  ...
                  >
                    <div className="relative h-44 bg-surface">
                      ...
                      <div className="absolute top-3 left-3 flex gap-2">
                        <span className="bg-[var(--lp-ink)] text-white text-xs font-semibold px-2 py-1 rounded">Tour</span>
                        {pkg.featured && (
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-2">
                        ...
                        {pkg.slug ? (
                          <Link href={`/tours/${pkg.slug}${paxQS ? `?${paxQS}` : ""}`} className="text-sm font-semibold text-[var(--lp-brass)] hover:underline">
                            View Details →
                          </Link>
                        ) : (
                          <a

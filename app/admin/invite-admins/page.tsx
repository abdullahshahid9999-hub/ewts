...
function InviteAdminsInner() {
  ...
  async function revokeInvite(id: string, invEmail: string) {
    if (!confirm(`Revoke invite for ${invEmail}? This will delete their pending account.`)) return;
    const res = await adminFetch("/api/admin/invite", accessToken, refresh, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <AdminShell>
      <div className="adp-ph"><div><h2>Invite <em>Admins</em></h2><p>Invite new admin users via a secure one-time email link</p></div></div>
      {/* ── Invite Form ── */}
      <div className="adp-card" style={{ padding: 24, marginBottom: 28 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 12, alignItems: "flex-end" }}>
          ...
          <button
            onClick={sendInvite} disabled={sending || !email}
        ...
        <p style={{ marginTop: 14, marginBottom: 0, fontSize: 12, color: "var(--a-muted)" }}>
          The invited admin will receive an email with a secure link to set their password. Links expire after <strong>48 hours</strong>. You can resend by inviting the same email again.
        </p>
      </div>
      ...
      {loading ? (
        <p style={{ color: "var(--a-muted)", fontSize: 13 }}>Loading…</p>
      ) : pending.length === 0 ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1.5px solid var(--a-border)", background: "var(--a-raised)" }}>
                {["Email", "Name", "Invited By", "Expires", "Status", ""].map(h => (
              ...
              {pending.map((inv, i) => {
                ...
                return (
                  <tr key={inv.id} style={{ borderBottom: i < pending.length - 1 ? "1px solid var(--a-border)" : "none" }}>
                    <td style={{ padding: "11px 14px", fontSize: 13, fontWeight: 600 }}>{inv.email}</td>
                    <td style={{ padding: "11px 14px", fontSize: 13, color: "var(--a-muted)" }}>{inv.fullName || "—"}</td>
                    <td style={{ padding: "11px 14px", fontSize: 12, color: "var(--a-muted)" }}>{inv.invitedByEmail || "—"}</td>
                    <td style={{ padding: "11px 14px", fontSize: 12, color: expired ? "var(--a-red)" : "var(--a-muted)" }}>

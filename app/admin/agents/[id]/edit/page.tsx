...
type Agent = { id: string; agentCode: string; fullName: string; email: string; phone: string | null; balance: number; creditLimit: number; tier: string; status: string; agencyName?: string | null; agencyAddress?: string | null; dtsLicense?: boolean; dtsLicenseNumber?: string | null; commissionRates: CommissionRate[] };
...
const SERVICE_TYPES = [{ value:"umrah",label:"Umrah" },{ value:"group_ticket",label:"Group Ticket" },{ value:"insurance",label:"Insurance" },{ value:"world_tour",label:"World Tour" },{ value:"visa_services",label:"Visa Services" }];
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:10 }}>
      {(Object.keys(PERM_LABELS) as Perm[]).map(k => (
        <label key={k} style={{ display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}>
          ...
          <div><div style={{ fontSize:13, fontWeight:500 }}>{PERM_LABELS[k].label}</div><div style={{ fontSize:11, opacity:0.55 }}>{PERM_LABELS[k].desc}</div></div>
        </label>
      ))}
      <div style={{ gridColumn:"1/-1", fontSize:11, opacity:0.4, marginTop:2 }}>⚠ Issue Tickets — sirf agency owner (main login) kar sakta hai. Sub-users ke liye hamesha OFF.</div>
    </div>
  );
}
...
function EditAgentInner() {
  ...
  const load = useCallback(async () => {
    ...
    if (a) { setAgent(a); setForm({ fullName:a.fullName, phone:a.phone??"", balance:String(a.balance), creditLimit:String(a.creditLimit), tier:a.tier, status:a.status, agencyName:a.agencyName??"", agencyAddress:a.agencyAddress??"" }); setDtsLicense(a.dtsLicense??false); setDtsNumber(a.dtsLicenseNumber??""); }
    setLoading(false); loadSubs();
  ...
  async function saveAgent(e: React.FormEvent) {
    ...
    const res = await adminFetch(`/api/admin/agents/${id}`, accessToken, refresh, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ ...form, balance:Number(form.balance), creditLimit:Number(form.creditLimit), dtsLicense, dtsLicenseNumber:dtsLicense?dtsNumber:"" }) });
    setSaving(false);
    if (res.ok) { flash("Saved ✓"); load(); } else flash((await res.json().catch(()=>({}))).error??"Error", false);
  }

  async function saveRate(e: React.FormEvent) {
    e.preventDefault(); if (!rateForm.value) return;
  ...
  async function saveSub(userId: string) {
    const res = await adminFetch(`/api/admin/agents/${id}/sub-users/${userId}`, accessToken, refresh, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ permissions:editPerms }) });
  ...
  return (<>
    ...
    <Card title="Agency Info" sub="Name, contact, DTS license, tier, status">
      <form onSubmit={saveAgent} className="adp-fg adp-fr">
        ...
        <div style={{ gridColumn:"1/-1" }}><label>Agency Name</label><input value={form.agencyName} onChange={e=>setForm(f=>({...f,agencyName:e.target.value}))} placeholder="e.g. Al-Noor Travels" /></div>
        <div style={{ gridColumn:"1/-1" }}><label>Agency Address</label><textarea style={{ resize:"vertical", minHeight:60, width:"100%" }} value={form.agencyAddress} onChange={e=>setForm(f=>({...f,agencyAddress:e.target.value}))} /></div>
        <div style={{ gridColumn:"1/-1" }}>
          <label>DTS License</label>
          <div style={{ display:"flex", gap:8, marginTop:4 }}>
            {[true,false].map(v=>(
              <button key={String(v)} type="button" onClick={()=>setDtsLicense(v)} style={{ padding:"7px 18px", borderRadius:8, fontWeight:700, fontSize:13, cursor:"pointer", border:"2px solid", borderColor:dtsLicense===v?"var(--a-gold)":"var(--a-border)", background:dtsLicense===v?"var(--a-gold)":"transparent", color:dtsLicense===v?"#fff":"var(--a-text)" }}>
                {v?"✓ Yes":"✗ No"}
              </button>
            ))}
          </div>
        </div>
        {dtsLicense && <div style={{ gridColumn:"1/-1" }}><label>DTS License Number</label><input value={dtsNumber} onChange={e=>setDtsNumber(e.target.value)} placeholder="DTS-2024-00123" /></div>}
    ...
    <Card title="Commission Rates" sub="Har service per agent ko kitna commission milta hai">
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:8, marginBottom:16 }}>
        {SERVICE_TYPES.map(st=>{
          const r=agent.commissionRates.find(x=>x.serviceType===st.value);
          return <div key={st.value} style={{ background:"var(--a-surface-2,rgba(255,255,255,0.04))", borderRadius:8, padding:"10px 14px", border:"1px solid var(--a-border)" }}>
            <div style={{ fontSize:11, opacity:0.6, marginBottom:4 }}>{st.label}</div>
            {r?<div style={{ fontWeight:600, fontSize:15 }}>{r.rateType==="percentage"?`${r.value}%`:`PKR ${r.value.toLocaleString()}`}<span style={{ fontSize:10, opacity:0.45, marginLeft:4 }}>{r.rateType}</span></div>:<div style={{ opacity:0.35, fontSize:13 }}>Not set</div>}
          </div>;
        })}
      </div>
      <form onSubmit={saveRate} style={{ display:"flex", flexWrap:"wrap", gap:8, alignItems:"flex-end" }}>
        <div><label style={{ fontSize:11, opacity:0.6, display:"block", marginBottom:4 }}>Service</label><select value={rateForm.serviceType} onChange={e=>setRateForm(f=>({...f,serviceType:e.target.value}))} style={{ width:"auto" }}>{SERVICE_TYPES.map(s=><option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
        ...
        <div key={u.id} style={{ background:"var(--a-surface-2,rgba(255,255,255,0.04))", border:"1px solid var(--a-border)", borderRadius:10, padding:"14px 16px", marginBottom:10 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              <button className="adp-btn adp-btn-s" onClick={()=>{ setEditId(editId===u.id?null:u.id); setEditPerms({...DEFAULT_PERMS,...u.permissions}); }}>{editId===u.id?"Cancel":"Edit Permissions"}</button>
        ...
        <div style={{ background:"var(--a-surface-2,rgba(255,255,255,0.04))", border:"1px solid var(--a-border)", borderRadius:10, padding:16 }}>
          ...
          <form onSubmit={createSub}>
            <div className="adp-fg adp-fr" style={{ marginBottom:14 }}>
              ...
              <div><label>Designation <span style={{ fontSize:11, opacity:0.5 }}>(e.g. Sales Manager)</span></label><input value={newU.designation} onChange={e=>setNewU(f=>({...f,designation:e.target.value}))} /></div>
              <div style={{ gridColumn:"1/-1" }}><label>Password (min 8)</label><input type="password" value={newU.password} onChange={e=>setNewU(f=>({...f,password:e.target.value}))} required minLength={8} /></div>
            </div>

...
async function getPackage(slug: string) {
  try {
    return await prisma.package.findFirst({
      where: { slug, category: "tours", status: "active" },
...
export default async function AgentToursDetailPage({ params }: { params: Promise<{ slug: string }> }) {
      ...
      <AgentShell>
        <div className="ap-ph">
          <div>
            ...
            <p>
              <Link href="/agent/tours" style={{ color: "var(--muted)" }}>← Back to World Tours</Link>
        ...
        <AgentPackageBookingWidget packageId={pkg.id} roomTypes={pkg.roomTypes} category="tours" />
      </AgentShell>
    </AgentGuard>
  );
}

...
export default function FilterSidebar({
  ...
  return (
    <>
      ...
      <div className="lg:hidden flex items-center justify-between gap-3 mb-5 w-full">
        <button
          onClick={openMobile}
          className="inline-flex items-center gap-2 bg-white border border-[var(--lp-border)] rounded-xl px-4 py-2.5 text-sm font-semibold shadow-sm"
        >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[999] flex flex-col bg-white lg:hidden" style={{ WebkitOverflowScrolling: "touch" }}>
          {/* Header */}
          <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[var(--lp-border)] shrink-0">
            <div>
            ...
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 text-xl font-light transition-colors"
              aria-label="Close"
            >
              ✕
            </button>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5">
            {draftActiveCount > 0 && (
              <button
                ...
                className="text-xs font-semibold text-red-500 hover:text-red-600 mb-5 block"
              >
                ✕ Clear all filters
              </button>
            )}
          ...
          {/* Footer: Update Now */}
          <div className="shrink-0 px-5 py-4 border-t border-[var(--lp-border)] bg-white safe-bottom">
            <button
              onClick={applyAndClose}
              className="w-full bg-[var(--lp-brass)] hover:bg-[var(--lp-brass-light)] text-black font-bold py-4 rounded-2xl text-sm transition-colors"
            >
              {draftActiveCount > 0

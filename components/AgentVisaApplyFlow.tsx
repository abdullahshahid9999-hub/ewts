...
// ── STEP INDICATOR ────────────────────────────────────────────────────────────
const STEPS = ["Travellers", "Contact", "Details & Docs", "Review"];
          ...
          <div key={label} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : undefined }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{
                ...
                background: done ? "var(--gold)" : active ? "var(--navy,#0A1930)" : "var(--bdr,#e5e7eb)",
                color: done || active ? "#fff" : "var(--muted)",
                border: active ? "2px solid var(--navy,#0A1930)" : "none",
              }}>
                {done ? "✓" : i + 1}
              </div>
            ...
            {i < STEPS.length - 1 && (
              <div style={{ flex: 1, height: 2, background: done ? "var(--gold)" : "var(--bdr,#e5e7eb)", margin: "0 8px", marginBottom: 20 }} />
            )}
          </div>
        );
      })}
...
function Counter({ label, sub, value, min, onChange }: { label: string; sub?: string; value: number; min: number; onChange: (v: number) => void }) {
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0, border: "1px solid var(--bdr)", borderRadius: 10, overflow: "hidden" }}>
        <button type="button" onClick={() => onChange(Math.max(min, value - 1))}
          style={{ width: 44, height: 44, fontWeight: 700, fontSize: 20, border: "none", background: "var(--bg)", cursor: "pointer", color: value <= min ? "var(--muted)" : "inherit" }}>−</button>
        <span style={{ minWidth: 44, textAlign: "center", fontWeight: 700, fontSize: 18, background: "#fff" }}>{value}</span>
        <button type="button" onClick={() => onChange(value + 1)}
          style={{ width: 44, height: 44, fontWeight: 700, fontSize: 20, border: "none", background: "var(--bg)", cursor: "pointer" }}>+</button>
      </div>
    </div>
  );
}
...
export default function AgentVisaApplyFlow({ visa }: { visa: VisaInfo }) {
  ...
  // step 1=travellers, 2=contact, 3=details+docs, 4=review, 5=done
  ...
  if (step === 5 && result) {
    return (
      <div className="ap-card" style={{ padding: 40, textAlign: "center", maxWidth: 520, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 20 }}>No payment collected yet — this is a booking request. Admin will review and follow up.</p>
        <button onClick={() => { setStep(1); setPhone(""); setEmail(""); setCounts({ adults: 1, children: 0, infants: 0 }); setTravellers([]); setResult(null); }} className="ap-btn ap-btn-ghost">
    ...
    <div className="ap-card" style={{ padding: "28px 28px 24px" }}>
      ...
      {step === 1 && (
        <>
          ...
          <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 28 }}>Select the number of adults, children, and infants applying for this visa.</p>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, maxWidth: 480, margin: "0 auto 32px" }}>
          ...
          <div style={{ background: "var(--bg)", border: "1px solid var(--bdr)", borderRadius: 12, padding: "16px 20px", maxWidth: 480, margin: "0 auto 28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
            ...
            {hasPricing && (
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 17, color: "var(--gold)" }}>
                  PKR {(counts.adults * (visa.priceAdult ?? 0) + counts.children * (visa.priceChild ?? 0) + counts.infants * (visa.priceInfant ?? 0)).toLocaleString()}
                </div>
              </div>
            )}
          </div>

          <button onClick={() => { buildTravellers(); setStep(2); }} className="ap-btn ap-btn-gold" style={{ width: "100%", padding: "14px", fontSize: 15 }}>
      ...
      {step === 2 && (
        <>
          <h3 style={{ fontWeight: 700, fontSize: 17, marginBottom: 4 }}>Customer Contact</h3>
          <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 24 }}>Who is the primary contact for this application?</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24, maxWidth: 560 }}>
            <div className="ap-field">
              <label>Phone Number</label>
              <input required type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="03xx-xxxxxxx" />
            </div>
            <div className="ap-field">
              <label>Email Address</label>
              <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="customer@email.com" />
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => {
              ...
              Continue — Traveller Details →
            </button>
          </div>
        </>
      )}

      {/* ── STEP 3: TRAVELLER DETAILS + DOCS ── */}
        ...
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {travellers.map((t, i) => {
                const filled = !!t.fullName.trim();
                return (
                  <button key={i} type="button" onClick={() => setActiveTrav(i)}
                    style={{ padding: "6px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, border: "1.5px solid", cursor: "pointer",
                      borderColor: i === activeTrav ? "var(--navy,#0A1930)" : filled ? "var(--gold)" : "var(--bdr)",
                      background: i === activeTrav ? "var(--navy,#0A1930)" : "transparent",
                      color: i === activeTrav ? "#fff" : filled ? "var(--gold)" : "var(--muted)" }}>
                    {filled ? "✓ " : ""}{i + 1}. {t.ageGroup.charAt(0).toUpperCase() + t.ageGroup.slice(1)}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div className="ap-field" style={{ gridColumn: "1/-1" }}>
              <label>Full Name <span style={{ color: "var(--red)" }}>*</span> <span style={{ fontWeight: 400, fontSize: 11, color: "var(--muted)" }}>(exactly as on passport)</span></label>
              <input required value={travellers[activeTrav].fullName} onChange={e => updateTrav(activeTrav, { fullName: e.target.value })} placeholder="e.g. Muhammad Ali" />
            </div>
            <div className="ap-field">
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 10, marginBottom: 16 }}>
                {filterDocsForApplicant(visa.requiredDocuments, travellers[activeTrav].applicantCategory, travellers[activeTrav].nationality).map((doc) => {
                  ...
                  return (
                    <div key={doc.id} style={{ border: `1.5px solid ${f ? "#16a34a" : doc.isRequired ? "var(--bdr)" : "var(--bdr)"}`, borderRadius: 10, padding: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ fontSize: 12.5, fontWeight: 600 }}>
                          {doc.name}
                          {doc.isRequired ? <span style={{ color: "var(--red)", marginLeft: 6, fontSize: 10 }}>*required</span>
                            : <span style={{ color: "var(--muted)", marginLeft: 6, fontSize: 10 }}>(optional)</span>}
                        </span>
      ...
      {step === 4 && (
        <>
          ...
          {/* Contact summary */}
          <div style={{ background: "var(--bg)", border: "1px solid var(--bdr)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 24, flexWrap: "wrap", fontSize: 12 }}>
            <div><span style={{ color: "var(--muted)" }}>Phone: </span><strong>{phone}</strong></div>
            <div><span style={{ color: "var(--muted)" }}>Email: </span><strong>{email}</strong></div>
            <div><span style={{ color: "var(--muted)" }}>Travellers: </span><strong>{travellers.length}</strong></div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 10, marginBottom: 16 }}>
            {travellers.map((t, i) => {
              ...
              const reqDocs = visa.requiredDocuments.filter(d => d.isRequired).length;
              const allDone = docCount >= reqDocs;
              return (
                <div key={i} style={{ border: `1.5px solid ${allDone ? "#16a34a" : "var(--bdr)"}`, borderRadius: 10, padding: "12px 14px" }}>
                  ...
                  <div style={{ fontSize: 11, marginTop: 4, color: allDone ? "#16a34a" : "var(--muted)" }}>📎 {docCount} doc{docCount !== 1 ? "s" : ""} attached</div>
                  <button type="button" onClick={() => { setActiveTrav(i); setStep(3); }} style={{ fontSize: 11, color: "var(--gold)", background: "none", border: "none", cursor: "pointer", padding: "4px 0", textDecoration: "underline" }}>Edit</button>
                </div>
              );
            })}
          </div>
          ...
          {hasPricing && (
            <div style={{ background: "var(--bg)", border: "2px solid var(--gold)", borderRadius: 12, padding: "14px 20px", marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setStep(3); setError(null); }} className="ap-btn ap-btn-ghost">← Back</button>
            <button onClick={handleSubmit} disabled={submitting} className="ap-btn ap-btn-gold" style={{ flex: 1, padding: "14px", fontSize: 15 }}>
